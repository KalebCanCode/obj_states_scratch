...
n_vid_path = 'www.youtube.com/watch?v=OFR57T1G1yM'
yt=YouTube(n_vid_path,on_progress_callback=on_progress)
videos=yt.streams.filter(file_extension = "mp4").first()
filename = "apple_test.mp4"
videos.download(output_path=data_dir, filename=filename)
...
def get_frame_indicies(sal: list, frames):
    indicies = ()
    label = 0
    orderings = increasing_sets(frames)
    s1_vector = model_clip.encode_text(clip.tokenize([sal[0]]).to(device))
    a_vector = model_clip.encode_text(clip.tokenize([sal[1]]).to(device))
    s2_vector = model_clip.encode_text(clip.tokenize([sal[2]]).to(device))
